...
function VirtualGuests() {
    const GuestsString = localStorage.getItem("VirtualGuests");
    const GuestsDetails = JSON.parse(GuestsString) || [];

    const EventsString = localStorage.getItem("VirtualEvents");
            ...
            {GuestsDetails && GuestsDetails.length > 0 ? (
                <nav>
                     <aside className="aside">
                        ...
                        <h1>
                            <Link to={'/virtualGuests'} className={`${window.location.pathname === '/virtualGuests' ? 'active' : ''}`}>VIRTUAL</Link>
                        </h1>
                        <h1>
                            <Link to={'/hybridGuests'}>HYBRID</Link>
                        </h1>
                     </aside>
                        <div className="TableComponent">
                            <h1>Virtual Event Guests</h1>
